...
export default function HealthInsurancePage() {
  ...
  const handleBuyPlan = (name: string, premium: number, vat: number, total: number) => {
  ...
  const handleCheckoutSubmit = (e: React.FormEvent) => {
    ...
    if (fullName && nidNumber && mobileNumber && nomineeMobile) {
      setTransactionId(`GD-HLTH-${Math.floor(100000 + Math.random() * 900000)}`);
              ...
              <div className="lg:col-span-5 flex flex-col justify-between p-8 bg-white rounded-3xl border border-slate-150 shadow-xl relative overflow-hidden">
                ...
                <div className="mt-8">
                  <button
                    ...
                    className="w-full text-center text-xs font-bold bg-brand-green hover:bg-brand-green-hover text-white py-3.5 px-4 rounded-xl hover:scale-[1.01] transition-all duration-300 shadow-md shadow-brand-green/10 cursor-pointer"
                  >
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-lg w-full overflow-hidden relative">
            ...
            <div className="p-6 bg-brand-blue text-white flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-slate-300 mt-0.5">Green Delta Health Protection Portal</p>
              </div>
              <button
              ...
              {!checkoutComplete ? (
                <form onSubmit={handleCheckoutSubmit} className="space-y-4">
                  ...
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between text-xs">
                    <div>
                      <span className="text-[10px] text-slate-400 block font-bold uppercase">Selected Plan:</span>
                      <span className="font-bold text-brand-blue text-sm">{selectedPlanName}</span>
                    </div>
                    <div className="text-right">
                      <span className="text-[10px] text-slate-400 block font-bold uppercase">Total Due:</span>
                      <span className="font-black text-brand-green text-sm">BDT {totalPrice}</span>
                    </div>
                  </div>
                ...
                <div className="text-center py-6 space-y-6">
                  ...
                  <div className="p-4 bg-slate-50 border border-slate-150 rounded-2xl text-left space-y-2.5 max-w-sm mx-auto text-xs">
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-400">Policy Package:</span>
                      <span className="font-bold text-slate-800">{selectedPlanName}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Base Premium:</span>
                      <span className="font-bold text-slate-850">BDT {selectedPlanPrice}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">VAT (15%):</span>
                      <span className="font-bold text-slate-850">BDT {vatPrice}</span>
                    </div>
                    <div className="flex justify-between pt-1.5 border-t border-slate-200">
                      <span className="text-slate-500 font-bold">Amount Paid:</span>
                      <span className="font-bold text-brand-green">BDT {totalPrice}</span>
                    </div>
                  </div>
